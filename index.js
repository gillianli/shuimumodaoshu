const MAGIC_VERSION = "2.3a";

let updateLog={
    "2023.4.23":["添加魔导书至2.5卷"],
	"2023.4.14":["原作者一直没有更新造成原版由于Chrome新规而无法使用，微调插件做到可用，并修改了原版法典导入后正面tag无法复位的bug"],
}
...
function initMagicBookGroup() {
    ...
    inputBtn.onclick = function () {
        ...
        toast("加载魔法书 " + book + " 成功，重新加载后生效", 2500);
    }
...
function initTopAlert(){
    ...
    alert.innerHTML+="魔导书船长水木版插件 "+MAGIC_VERSION+" 每次更新或删除本地缓存后显示该页面，点我关闭"+"<br/>";
...
function clearTextAreas() {
    // 获取正面文本框
    const positiveTextArea = document.getElementById("textarea_pos");
    // 清空正面文本框的内容
    positiveTextArea.value = "";

    // 获取负面文本框
    const negativeTextArea = document.getElementById("textarea_neg");
    // 清空负面文本框的内容
    negativeTextArea.value = "";
	localStorage.removeItem('textarea_pos');
    localStorage.removeItem('textarea_neg');
	localStorage.removeItem("checked_data");
    localStorage.removeItem("myWeight");
	uiConfig.select_magic = "";
        saveUiConfig();
        resetCheck(true, true);
}
